...
std::int_fast64_t
TransOffset(bool leap_year, int jan1_weekday, const PosixTransition& pt) {
...
inline time_zone::civil_lookup
MakeSkipped(const Transition& tr, const civil_second& cs) {
  time_zone::civil_lookup cl;
...
inline time_zone::civil_lookup
MakeRepeated(const Transition& tr, const civil_second& cs) {
  time_zone::civil_lookup cl;
...
bool TimeZoneInfo::Header::Build(const tzhead& tzh) {
  ...
  if ((v = Decode32(tzh.tzh_ttisgmtcnt)) < 0)
    return false;
...
void TimeZoneInfo::CheckTransition(
    const std::string& name,
...
bool TimeZoneInfo::EquivTransitions(
...
void TimeZoneInfo::ExtendTransitions(
...
bool TimeZoneInfo::Load(const std::string& name, ZoneInfoSource* zip) {
  ...
  while (hdr.timecnt > 1) {
    if (!EquivTransitions(
            transitions_[hdr.timecnt - 1].type_index,
...
std::unique_ptr<ZoneInfoSource>
FileZoneInfoSource::Open(const std::string& name) {
...
TimeZoneInfo::TimeLocal(const civil_second& cs, cctz::year_t c4_shift) const {
...
TimeZoneInfo::BreakTime(const time_point<sys_seconds>& tp) const {
  ...
  const Transition* tr = std::upper_bound(
      begin, begin + timecnt, target, Transition::ByUnixTime());
  local_time_hint_.store(
      static_cast<std::size_t>(tr - begin), std::memory_order_relaxed);
...
time_zone::civil_lookup TimeZoneInfo::MakeTime(const civil_second& cs) const {
  ...
  if (tr == begin) {
    if (tr->prev_civil_sec >= cs) {
      ...
      if (cs < tt.civil_min)
        return MakeUnique(time_point<sys_seconds>::min());
  ...
  if (tr == end) {
    if (cs > (--tr)->prev_civil_sec) {
      ...
      const TransitionType& tt(transition_types_[tr->type_index]);
      if (cs > tt.civil_max)
        return MakeUnique(time_point<sys_seconds>::max());
...
bool TimeZoneInfo::NextTransition(time_point<sys_seconds>* tp) const {
  ...
  if (tr != begin) { // skip no-op transitions
    for (; tr != end; ++tr) {
      if (!EquivTransitions(tr[-1].type_index, tr[0].type_index))
        break;
    }
  }
...
bool TimeZoneInfo::PrevTransition(time_point<sys_seconds>* tp) const {
  ...
  if (tr != begin) { // skip no-op transitions
    for (; tr - 1 != begin; --tr) {
      if (!EquivTransitions(tr[-2].type_index, tr[-1].type_index))
        break;
    }
  }
